...
이 프로젝트는 OpenAI gpt-4o-mini를 기반으로 LangGraph 구조와 RAG(Retrieval-Augmented Generation)를 활용하여 구현된 영화 정보 검색 및 추천 대화형 에이전트입니다.

사용자의 질문 의도를 파악하여 RAG를 통해 영화 정보를 검색하거나, 도구(Tool)를 활용해 OTT 시청 가능 여부 확인, 계산, 시간 확인 등의 작업을 수행합니다. 또한, 장기/단기 메모리 및 Reflection 시스템을 통해 개인화된 대화 경험을 제공합니다.

## 팀원 및 역할 (Team Members)
| 학번 | 이름 | 역할 | 담당 업무 |
|:---:|:---:|:---:|:---|
| 2021128007 | **김정훈** (팀장) | RAG | RAG 시스템 구축, 벡터 검색 |
| 2021125017 | **김지홍** | Memory | Memory 시스템(단기/장기/Reflection) |
| 2023125065 | **홍지호** | Tools | 일반 Tool 구현(OTT, 계산 등) |
...
### 3. Memory 시스템
* **Short-term:** LangGraph State + MemorySaver (세션 유지) [cite: 63-67]
* **Long-term:** ChromaDB Persistent (중요 대화 영구 저장) [cite: 68-70]
* **Reflection:** 도구 사용(+0.3), RAG 사용(+0.2) 등의 조건으로 중요도 판단 및 자동 저장 [cite: 74-82]
