...
def should_save_memory(state: Dict[str, Any]) -> bool:
    ...
    if not has_final_answer or not has_user_query:
        return False
...
def reflect_and_save(state: Dict[str, Any]) -> Optional[str]:
    ...
    if importance < 0.3:
        print(f"[MEMORY] ❌ 저장 스킵: 중요도 부족")
        return None

    # 장기 메모리에 저장
    print(f"[MEMORY] 메모리 저장 시도 (중요도: {importance:.2f})")
    try:
        ...
        print(f"[MEMORY] ✅ 저장 성공: {memory_id[:30]}...")
        return memory_id
    except Exception as e:
        print(f"[MEMORY] ❌ 저장 스킵: 저장 중 오류 발생")
        return None
...
def get_relevant_memories(query: str, top_k: int = 3) -> list:
    ...
    try:
        ...
        print(f"[MEMORY] 관련 메모리 검색: {len(memories)}개 발견")
        return memories
    except Exception as e:
        return []
